...
TEST_CASE("Board is correctly converted to bitboard"){
    ...
    SECTION("Harder tests")
    {
        ...
        uint64_t expectedMask = calcPowersOfTwo({14,15,16,21,22,23,28,29,30});
...
TEST_CASE("canPlay returns false when column is full"){
    ...
    SECTION("Edge cases"){
        ...
        for(int i = 0; i < 7; ++i) { REQUIRE(bitBoard.canPlay(i)); }
    ...
    board.reset();
    for(int i = 0; i < 7; ++i) { board.addDisc(Disc::Colour::white,0); }
    bitBoard.convertToBitboard(board,Disc::Colour::black);

    REQUIRE(!bitBoard.canPlay(0));
    for(int i = 1; i < 7; ++i) { REQUIRE(bitBoard.canPlay(i)); }

    for(int i = 0; i < 7; ++i) { board.addDisc(Disc::Colour::white,6); }
...
TEST_CASE("Check win detects wins"){
    ...
    SECTION("Vertical wins"){
        ...
        for(int i = 0; i < 4; ++i) { board.addDisc(Disc::Colour::white, nCols-1); }
        bitBoard.convertToBitboard(board,Disc::Colour::white);
        REQUIRE(bitBoard.checkWin(bitBoard.getCurrentBoard()));
        board.reset();

        for(int i = 0; i < 4; ++i) { board.addDisc(Disc::Colour::black, nCols-1); }
        bitBoard.convertToBitboard(board,Disc::Colour::black);
        REQUIRE(bitBoard.checkWin(bitBoard.getCurrentBoard()));
        board.reset();
        ...
        board.addDisc(Disc::Colour::white, 0);
        board.addDisc(Disc::Colour::white, 0);
        for(int i = 0; i < 4; ++i) { board.addDisc(Disc::Colour::black, 0); }
        bitBoard.convertToBitboard(board,Disc::Colour::black);
        REQUIRE(bitBoard.checkWin(bitBoard.getCurrentBoard()));
    }
    ...
    SECTION("Horizontal wins"){
        board.reset();
        for(int i = 0; i < 4; ++i) { board.addDisc(Disc::Colour::white, i); }
        bitBoard.convertToBitboard(board,Disc::Colour::white);
        REQUIRE(bitBoard.checkWin(bitBoard.getCurrentBoard()));
        board.reset();

        for(int i = 2; i < 6; ++i) { board.addDisc(Disc::Colour::white, i); }
        bitBoard.convertToBitboard(board,Disc::Colour::white);
        REQUIRE(bitBoard.checkWin(bitBoard.getCurrentBoard()));
        board.reset();
        ...
        for(int i = 0; i < 4; ++i){ board.addDisc(Disc::Colour::black, i);}
        bitBoard.convertToBitboard(board,Disc::Colour::black);
        REQUIRE(bitBoard.checkWin(bitBoard.getCurrentBoard()));
        board.reset();
...
TEST_CASE("isWinningMove returns true when next move is a win"){
    ...
    Bitboard bitboard;
    for(int i = 0; i < 3; i++) { bitboard.addDisc(0); }
    bitboard.updateBoard();
    REQUIRE(bitboard.isWinningMove(0));

    bitboard.reset();
    for(int i = 0; i < 3; i++) { bitboard.addDisc(1); }
    bitboard.updateBoard();
    REQUIRE(!bitboard.isWinningMove(0));
...
TEST_CASE("Win is detected after adding discs to columns"){
    ...
    SECTION("Simple column win test using update board"){
        Bitboard bitboard;
        for(int i = 0; i < 4; i++) { bitboard.addDisc(0); }
        bitboard.updateBoard();
        REQUIRE(bitboard.checkWin(bitboard.getCurrentBoard()));
    }
...
/*
TEST_CASE("Discs are correctly removed after being added"){
    SECTION("one disc"){
        Board board;
        Bitboard bitboard;
        board.addDisc(Disc::Colour::white, 0);
        ...
        bitboard.updateBoard();
        //board.takeTurn(0);
        auto currentBoard{bitboard.getCurrentBoard()};
        auto mask{bitboard.getMask()};

        bitboard.reverseTurn(0);
        REQUIRE(currentBoard == bitboard.getCurrentBoard());
        REQUIRE(mask == bitboard.getMask());
    }
